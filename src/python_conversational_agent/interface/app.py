...
from python_conversational_agent.interface.component_decl import threejs_mascot_component
...
MASCOT_GLB_PATH = Path(__file__).parent / "mascot_frontend" / "mascot.glb"

USE_3D = MASCOT_GLB_PATH.exists()

if not USE_3D:
    mascot_2d = MascotController(assets_dir=ASSETS_MASCOT_DIR)


def render_mascot(emotion: str, emotion_id: int):
    if USE_3D:
        # Render the custom Three.js mascot component passing the active emotion and event ID
        threejs_mascot_component(emotion=emotion, emotion_id=emotion_id, key="mascot_comp")
    else:
...
if "emotion_id" not in st.session_state:
    st.session_state.emotion_id = 0
if "waiting_for_response" not in st.session_state:
    st.session_state.waiting_for_response = False

with st.sidebar:

    render_mascot(st.session_state.emotion, st.session_state.emotion_id)
